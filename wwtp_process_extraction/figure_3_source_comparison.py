...
def build_major_category_sources(common_facilities, include_kw=False):
    ...
    if include_kw:
        ...
        for label, cwns_c, llm_c, kw_c in zip(cat_labels, cwns_counts, llm_counts, kw_counts):
            cwns_detected = sum(cwns_c.get(k, 0) for k in ("PRESENT", "PAST", "FUTURE", "OFFSITE"))
            llm_detected = sum(llm_c.get(k, 0) for k in ("PRESENT", "PAST", "FUTURE", "OFFSITE"))
            kw_detected = sum(kw_c.get(k, 0) for k in ("PRESENT", "PAST", "FUTURE", "OFFSITE"))
            if cwns_detected >= MAJOR_MIN_COUNT or llm_detected >= MAJOR_MIN_COUNT or kw_detected >= MAJOR_MIN_COUNT:
                kept.append((label, cwns_c, llm_c, kw_c))
        if kept:
    ...
    for label, cwns_c, llm_c in zip(cat_labels, cwns_counts, llm_counts):
        cwns_detected = sum(cwns_c.get(k, 0) for k in ("PRESENT", "PAST", "FUTURE", "OFFSITE"))
        llm_detected = sum(llm_c.get(k, 0) for k in ("PRESENT", "PAST", "FUTURE", "OFFSITE"))
        if cwns_detected >= MAJOR_MIN_COUNT or llm_detected >= MAJOR_MIN_COUNT:
            kept.append((label, cwns_c, llm_c))
    if kept:
...
def build_sorted_plot_items(json_cats, keywords, cwns_df, llm_df):
    ...
    for cat_name in json_cats:
        if cat_name not in keywords:
            continue
        leaves = get_leaf_names(cat_name, keywords[cat_name], exclude_unspecified=True)
        for leaf in leaves:
    ...
    for item, label, cwns_c, llm_c in zip(items, labels, cwns_counts_list, llm_counts_list):
        cwns_detected = sum(cwns_c.get(k, 0) for k in ("PRESENT", "PAST", "FUTURE", "OFFSITE"))
        llm_detected = sum(llm_c.get(k, 0) for k in ("PRESENT", "PAST", "FUTURE", "OFFSITE"))
        if cwns_detected >= MIN_COUNT or llm_detected >= MIN_COUNT:
            kept.append((item, label, cwns_c, llm_c))
    if not kept:
        return [], [], [], []
    tuples = list(kept)
    result = []
...
for group_title, json_cats in PLOT_GROUPS.items():
    ...
    # Exclude any leaf with <5 instances in BOTH sources
    filtered = []
    for item, label, cwns_c, llm_c in zip(items, labels, cwns_counts_list, llm_counts_list):
        if (sum(cwns_c.values()) < 5) and (sum(llm_c.values()) < 5):
            continue
        filtered.append((item, label, cwns_c, llm_c))
    if filtered:
        items, labels, cwns_counts_list, llm_counts_list = map(list, zip(*filtered))
    else:
    ...
    render_source_plot(
        ...
        source_items=[("Clean Watershed Needs Survey", "Clean Watershed Needs Survey"), ("Facility Permit - LLM extraction", "npdes_llm")],
        ...
        groups=[
            {
                "header": "Data Source",
                "items": [
                    ("Clean Watershed Needs Survey", {"facecolor": COLORS["Clean Watershed Needs Survey"]}),
                    ("Facility Permit - LLM extraction", {"facecolor": COLORS["npdes_llm"]}),
                ],
            },
            {"header": "Status", "items": status_legend_items},
        ],
